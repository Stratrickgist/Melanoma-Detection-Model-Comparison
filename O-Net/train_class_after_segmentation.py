...
parser.add_argument('--end_epoch', '-e', default=75, type=int, help='end epoch')
...
train_imgs,val_imgs = './datasets/ISIC 2017/Training/ISIC-2017_Training_Data', './datasets/ISIC 2017/Validation/ISIC-2017_Validation_Data'
train_csv,val_csv = './datasets/ISIC 2017/Training/ISIC-2017_Training_Part3_GroundTruth_preprocessed.csv','./datasets/ISIC 2017/Validation/ISIC-2017_Validation_Part3_GroundTruth_preprocessed.csv'
...
def main():
    ...
    swin_multi_scale_dir = 'checkpoint/CNN_with_Swin_E_Swin_D/epoch_74.pth'  #loading segmentation weights
    model = model.to('cuda')
    model.load_from(swin_multi_scale_dir)
...
# Classification on test data
    test_imgs_dir = './datasets/ISIC 2017/Test/ISIC-2017_Test_v2_Data'
    test_csv = './datasets/ISIC 2017/Test/ISIC-2017_Test_v2_Part3_GroundTruth_preprocessed.csv'
    ...
    save_masks_dir = '/home/zombie/workspace/O-Net/checkpoint/classificationlr0.0001bs8/predicted_masks/'
    ...
    with torch.no_grad():
        ...
        for batch_idx, (imgs, masks) in enumerate(test_dl):
            ...
            predicted_labels = torch.argmax(masks_pred, 1).view(masks_cuda.size())  # Get predicted labels

            # Compute TP, FP, FN
            TP += ((predicted_labels == 1) & (masks_cuda == 1)).sum().item()
            FP += ((predicted_labels == 1) & (masks_cuda == 0)).sum().item()
            FN += ((predicted_labels == 0) & (masks_cuda == 1)).sum().item()
    ...
    print('Recall for Melanoma:', recall_class_1)
    print('Precision for Melanoma:', precision_class_1)
